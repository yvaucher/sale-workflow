...
class SaleOrder(models.Model):
    ...
    def detect_exceptions(self):
        ...
        order_exceptions = exception_obj.browse(order_exception_ids)
        line_exceptions = exception_obj.browse(line_exception_ids)
    ...
    @api.one
    def _exception_rule_eval_context(self, obj_name):
        user = self.env['res.users'].browse(self._uid)
        return {obj_name: self,
                'self': self.pool.get(self._name),
                'object': self,
                'obj': self,
                'pool': self.pool,
                'cr': self._cr,
    ...
    @api.one
    def _rule_eval(self, rule, obj_name, context):
        expr = rule.code
        space = self._exception_rule_eval_context(obj_name)
        try:
            safe_eval(expr,
                      space,
                      mode='exec',
        ...
        for order_line in self.order_line:
            for rule in line_exceptions:
                if rule.id in exception_ids:
                    ...
                    continue
                if order_line._rule_eval(rule, 'line'):
                    exception_ids.append(rule.id)
